...
void test(){
 ...
 // list[2][2] = 4;
  ...
  contract(list,nlist,n,clist,nc,ncmax,sgnflps);
}
...
int contract(int list[][], int nlist[], int n, int clist[][], int nc, int ncmax, bool sgnflps[]){
  ...
  while (i < n){
    fermsgn[i] = !fermsgn[i-1];
    i++;
  }

  cdrive(idx,list,nlist,n,nc,clist,ncmax,seq,iseq,sgnflps,fermsgn,sgnflp);
...
int cdrive(int n, int nc, int ncmax, int idx, int list[n][n], int nlist[n], int clist[n][ncmax], int seq[2][n], int iseq, bool sgnflps[ncmax], bool fermsgn[n], bool sgnflp[n]){
  ...
  if ((2 * iseq) == n){
    nc++;
    if (nc > ncmax){
      return 1; // number of contractions exceeded
    }
    int i = 0;
    while(i < iseq){
      clist[seq[1][i]][nc] = seq[2][i];
      sgnflps[nc] = sgnflps[nc] != sgnflp[i];
      i++;
    }
    return 0;
  }
  ...
  while (i < iseq){
    if (idx == seq[2][i]){
      lnew = false; //already in sequence
      break;
    }
    i++;
  }
  ...
  if (lnew) {
    ...
    int jseq = iseq + 1;
    seq[0][jseq] = idx;
    seq[1][jseq] = list[i][idx];

    int k = seq[0][jseq] + 1;
    while (k < seq[1][jseq] - 1){
      fermsgn[k] = !fermsgn[k];
      k++;
    }
    sgnflp[jseq] = (fermsgn[seq[0][jseq]] == fermsgn[seq[1][jseq]]);

    int jdx = idx + 1;
    cdrive(n,nc,ncmax,jdx,list,nlist,clist,seq,jseq,sgnflps,fermsgn,sgnflp);
